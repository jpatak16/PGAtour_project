...
---
```{r setup, echo=FALSE}
pacman::p_load(tidyverse, knitr, kableExtra, lubridate, binsreg, rsample, recipes, parsnip, workflows, tune, glmnet, ranger, parallel, vip, rvest)
...
```{r summarize results}
sims_results = data.frame(player = Masters22_preds$player, win_percent = NA, top10_percent = NA, make_cut_percent = NA)
for(p in 1:length(sims_results$player)){
  ...
  sims_results$win_percent[p] = win_percent
  top10_percent = (sims_p %>% filter(top10==TRUE) %>% nrow()) / nrow(sims_p)
  sims_results$top10_percent[p] = top10_percent
  make_cut_percent = (sims_p %>% filter(make_cut==TRUE) %>% nrow()) / nrow(sims_p)
  sims_results$make_cut_percent[p] = make_cut_percent
}
...
```{r plot results, out.width='33%'}
ggplot(sims_results %>% arrange(-win_percent) %>% head(n=10), aes(x = reorder(player, win_percent), win_percent)) +
  geom_bar(stat = 'identity', fill='darkgreen') + coord_flip() +
  xlab('Win Percentage') + ylab('Golfer') +
  ggtitle('Liklihood of Winning the 2022 Masters', 
          subtitle = 'According to 10,000 simulations')

ggplot(sims_results %>% arrange(-top10_percent) %>% head(n=20), aes(x = reorder(player, top10_percent), top10_percent)) +
  geom_bar(stat = 'identity', fill='yellow', color='darkgreen') + coord_flip() +
  xlab('Win Percentage') + ylab('Golfer') +
  ggtitle('Likihood of Finishing Top 10 at the 2022 Masters', 
          subtitle = 'According to 10,000 simulations')

ggplot(sims_results %>% arrange(-make_cut_percent) %>% head(n=20), aes(x = reorder(player, make_cut_percent), make_cut_percent)) +
  geom_bar(stat = 'identity', fill='darkgreen') + coord_flip() +
  xlab('Win Percentage') + ylab('Golfer') +
